    ...
    const getUser = async () => {
      if (donate.length > 0) {
        const kqUser = await Promise.all(donate.map(async (item) => {
          console.log("🚀 ~ file: DetailPage.js ~ line 126 ~ kqUser ~ item", parseInt(item.userID))
          return await handleGetAccountByID(
            `http://localhost:8080/api/auth/getAccountByID?accountId=${parseInt(item.userID)}`
          );
        }))
        console.log("🚀 ~ file: DetailPage.js ~ line 129 ~ kq ~ kq", kqUser)
      }
          ...
          <div className="w-[35%] flex flex-col justify-start pl-[20px]">
            <TableStyles>
              ...
              <table>
                ...
                <tbody>
                  {donate.length > 0 && donate.map((item) => (
                    <tr key={item?.id}>
