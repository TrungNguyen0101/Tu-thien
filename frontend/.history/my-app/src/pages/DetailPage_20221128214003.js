...
const DetailPage = () => {
  ...
  React.useEffect(() => {
    const getUser = async () => {
      if (donate.length > 0) {
        const kq = await Promise.all(donate.map(async (item) => {
          return await handleGetDonateByDonateId(
            `http://localhost:8080/api/donate/getDonateByDonateID?donateId=${item?.id}`
          );
        }))
        console.log("🚀 ~ file: DetailPage.js ~ line 129 ~ kq ~ kq", kq)
      }
