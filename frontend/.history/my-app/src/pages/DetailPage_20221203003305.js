...
const DetailPage = () => {
  ...
  const [<userPartic></userPartic>ipant, setUserParticipant] = useState({});
  console.log("🚀 ~ file: DetailPage.js:50 ~ DetailPage ~ userParticipant", userParticipant)
      ...
      if (participant?.length > 0) {
        const kq = await Promise.all(
          participant.map((item) => {
            console.log("🚀 ~ file: DetailPage.js:182 ~ participant.map ~ item", item)
            return handleGetUserParticipantByUserID(`http://localhost:8080/api/auth/getAccountByID?accountId=${item.userID}`)
          })
        )
