...
const DetailPage = () => {
  ...
  // console.log(
  //   "🚀 ~ file: DetailPage.js ~ line 43 ~ DetailPage ~ donate",
  //   donate
  // );
  // console.log("🚀 ~ file: DetailPage.js ~ line 39 ~ DetailPage ~ post", post)
  ...
  async function handleGetDonateByDonateId(url) {
    ...
    const result = await response.json();
    // console.log(
    //   "🚀 ~ file: DetailPage.js ~ line 78 ~ handleGetDonateByDonateId ~ result",
    //   result
    // );
    // setDonate(...result);
    return result[0];
  ...
  useEffect(() => {
    const getDonate = async () => {
      if (postDonate.length > 0) {
        ...
        console.log("🚀 ~ file: DetailPage.js ~ line 104 ~ kq ~ kq", kq)
        // const obj1 = {}
        // const check = kq.map((item) => {
        //   if (obj1.hasOwnProperty(item.userID)) {

        //     obj1[item.userID] += item.money
        //   }
        //   else {
        //     obj1[item.userID].money = item.money
        //   }
        //   return obj1
        // })
        // console.log(check);
        const result = kq.reduce((acc, curr) => {
          const objInAcc = acc.find((o) => o.userID === curr.userID && o.id !== curr.id);
          console.log("🚀 ~ file: DetailPage.js ~ line 120 ~ result ~ objInAcc", objInAcc)
          if (objInAcc) objInAcc.money += curr.money;
          else acc.push(curr);
          return acc;
        }, []);
        console.log("🚀 ~ file: DetailPage.js ~ line 124 ~ result ~ result", result)


        // postDonate.forEach((item) => {
