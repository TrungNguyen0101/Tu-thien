...
const DetailPage = () => {
  ...
  console.log("🚀 ~ file: DetailPage.js:47 ~ DetailPage ~ postComment", postComment)
      ...
      if (postComment?.length > 0) {
        const kq = await Promise.all(
          postComment.map(async (item) => {
            console.log("🚀 ~ file: DetailPage.js:261 ~ postComment.map ~ item", item)
            // return await handleGetCommentByCommentId(
            //   `http://localhost:8080/api/comment/getCommentByCommentID?commentId=${item.}`
            // );
          })
        );
  ...
  const submitForm = (values) => {
    ...
    handleAddComment("http://localhost:8080/api/comment/addComment", values)
