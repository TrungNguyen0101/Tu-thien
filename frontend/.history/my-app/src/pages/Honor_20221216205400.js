        ...
        const handleCountParticipantByUserID = async () => {
            if (account?.length > 0) {
                const kq = await Promise.all(
                    account.map((item) => {
                        return handleCallAPIGet(`http://localhost:8080/api/participant/getCountParticipantByUserId?userId=${item?.id}`)
                    })
                )
            ...
            if (account?.length > 0) {
                const kq = await Promise.all(
                    account.map(async (item) => {
                        ...
                        return await handleCallAPIGetPromise(`http://localhost:8080/api/participant/getCountParticipantByUserId?userId=${item?.id}`)
                    })
                )
                console.log("🚀 ~ file: Honor.js:76 ~ handleCountParticipantByUserID ~ kq", kq)
            }
