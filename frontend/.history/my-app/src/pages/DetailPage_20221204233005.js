...
const DetailPage = () => {
  ...
  const handleDeleteComment = (idUserComment) => {
    console.log("🚀 ~ file: DetailPage.js:348 ~ handleDeleteComment ~ idUserComment", idUserComment)
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (idUserComment === userId || userLocal.roleId === 3) {
          // const check = await handleDeleteAccountByID(`http://localhost:8080/api/comment/DeleteCommentById?id=12`)
                  ...
                  comment.map((item, index) => (
                    <div className="text-[13px] relative mb-[5px]" key={index}>
                      ...
                      <div
                        ...
                        onClick={handleDeleteComment(item.userID)}
                      >
