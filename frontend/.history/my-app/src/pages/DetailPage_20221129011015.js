...
const DetailPage = () => {
  ...
  console.log("🚀 ~ file: DetailPage.js ~ line 40 ~ DetailPage ~ relate", relate)
  ...
  const hot = post?.hot
  console.log("🚀 ~ file: DetailPage.js ~ line 47 ~ DetailPage ~ hot", hot)
  ...
  useEffect(() => {
    handleGetAllPostRelate(`http://localhost:8080/api/post/getAllPostRelate?hot=${hot}`)
  }, [hot])
  useEffect(() => {
    ...
    <div className="flex">
      <main className="">
        ...
        <div className="flex px-[40px] pt-[40px]">
          <div className="w-[60%] border-r-[2px] pr-[20px]">
          ...
          <div className="w-[40%] flex flex-col justify-start pl-[20px]">
            <TableStyles>
            ...
            <div className="">
              {relate?.length > 0 && relate.map((item) => (

                item.id !== parseInt(idPost) ? <ItemRelate key={item.id} item={item}></ItemRelate> : null
