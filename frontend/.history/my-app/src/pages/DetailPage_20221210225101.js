...
const DetailPage = () => {
  ...
  const handleUpdateFinish = async (url, data) => {
    try {
      ...
      const result = await response.json();
      console.log("🚀 ~ file: DetailPage.js:408 ~ handleUpdateFinish ~ result", result)
      toast.success(`${result?.message}`, {
        pauseOnHover: false,
        delay: 0,
        autoClose: 1300,
  ...
  return (
    <div className="flex">
      <main className="">
        ...
        <div className="m-auto mb-[20px]" onClick={() => handleUpdateFinish(`http://localhost:8080/api/participant/UpdateFinishByPostId?postId=${idPost}`)}>
