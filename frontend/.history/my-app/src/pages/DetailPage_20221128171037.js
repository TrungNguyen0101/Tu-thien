...
const DetailPage = () => {
  ...
  useEffect(() => {
    handleGetAllPost(`http://localhost:8080/api/post/getPostById?idPost=${post.id}`)
  }, [idPost])
  useEffect(() => {
    handleGetDonateIDByPostID(`http://localhost:8080/api/donate/getDonateIDByPostID?postId=${idPost}`)
  }, [idPost])
  useEffect(() => {
    handleGetDonateIDByPostID(
      `http://localhost:8080/api/donate/getDonateIDByPostID?postId=${postDonate.donateId}`
    );
