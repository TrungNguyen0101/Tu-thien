...
const DetailPage = () => {
  ...
  console.log("🚀 ~ file: DetailPage.js ~ line 42 ~ DetailPage ~ user", user)
  var nf = new Intl.NumberFormat();
  console.log("🚀 ~ file: DetailPage.js ~ line 41 ~ DetailPage ~ donate", donate)
  ...
  useEffect(() => {
    const getDonate = async () => {
      if (postDonate.length > 0) {
        const kq = await Promise.all(postDonate.map(async (item) => {
        ...
        let sortedMoney = result.sort(
          (p1, p2) => (p1.money < p2.money) ? 1 : (p1.money > p2.money) ? -1 : 0);
  ...
  useEffect(() => {
    const getUser = async () => {
      if (donate.length > 0) {
        const kqUser = await Promise.all(donate.map(async (item) => {
          return await handleGetAccountByID(
            `http://localhost:8080/api/auth/getAccountByID?accountId=${parseInt(item.userID)}`
          );
        }))
        setUser(kqUser)
      }
          ...
          <div className="w-[35%] flex flex-col justify-start pl-[20px]">
            <TableStyles>
              ...
              <table>
                ...
                <tbody>
                  {donate.length > 0 && donate.map((item, index) => (
                    <tr key={item?.id}>
                      <td>{user[index]?.displayName ? user[index]?.displayName : user[index]?.username}</td>
                      <td>{nf.format(item?.money.toString().replace(/\$|,|\./g, ""), 10) + " VNĐ"}</td>
                    </tr>
                  ))
                  }
