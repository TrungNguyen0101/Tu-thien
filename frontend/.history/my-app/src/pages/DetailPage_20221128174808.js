    ...
    const getDonate = async () => {
      if (postDonate.length > 0) {
        ...
        const result = kq.reduce((acc, curr) => {
          ...
          if (objInAcc) {
            (objInAcc.money) += parseInt(curr.money);
            console.log("🚀 ~ file: DetailPage.js ~ line 117 ~ result ~ objInAcc", objInAcc)
            console.log(typeof (objInAcc.money));
          }
          else acc.push(curr);
          return acc;
        }, []);
        console.log("🚀 ~ file: DetailPage.js ~ line 120 ~ result ~ result", result)
