...
const ReviewPost = (idPost) => {
    ...
    console.log("🚀 ~ file: ReviewPost.js:15 ~ ReviewPost ~ participant", participant)
    const userIDParticipant = participant?.userID
    console.log("🚀 ~ file: ReviewPost.js:17 ~ ReviewPost ~ userIDParticipant", userIDParticipant)
    const schema = yup
        .object({
    ...
    const submitReviewForm = (values) => {
        ...
        if (userLocal !== "") {
            if (comment.length > 0) {
                toast.error("Chỉ có người đăng mới được đánh giá hoặc bài viết đã đc đánh giá", {
                    pauseOnHover: false,
                    delay: 0,
                    autoClose: 1300,
                });
