...
const UpdateUser = () => {
    ...
    const schema = yup
        .object({
            ...
            email: yup.string().email("Vui lòng nhập đúng định dạng").required("Vui lòng nhập vào email").nullable(),
            code: yup.string().required("Vui lòng nhập vào vào mã code").nullable(),
        }).required()
    const {
        control,
        handleSubmit,
    ...
    }, [getValues, setValue, user?.code, user?.displayName, user?.email])
    ...
    const handleAddPost = (values) => {
        console.log("🚀 ~ file: UpdateUser.js:121 ~ handleAddPost ~ values", values)
        ...
        newValue.displayName = values.displayName || user?.displayName;
        newValue.email = values.email || user?.email;
        newValue.id = user?.id;
        handleUpdateAccount(
        ...
        console.log("🚀 ~ file: UpdateUser.js:136 ~ handleAddPost ~ newValue", newValue)
        // setTimeout(() => {
        //     navigate("/")
        //     // const reloadUsingLocationHash = () => {
        //     //     window.location.hash = "reload";
