...
const UpdateUser = () => {
    ...
    console.log("🚀 ~ file: updateUser.js ~ line 15 ~ UpdateUser ~ account", accountMoney)
    ...
    const newMoney = (parseInt(money) + parseInt(user.totalMoney)).toString()
    console.log("🚀 ~ file: updateUser.js ~ line 25 ~ UpdateUser ~ newMoney", newMoney)
    ...
    useEffect(() => {
        handleGetAccountByID(`http://localhost:8080/api/auth/getAccountByID?accountId=${user?.id}`);
    }, [user?.id]);
    const schema = yup
        .object({
    ...
    const handleAddPost = (values) => {
        ...
        var unformatted = money ? money.replace(/,/g, "") : user?.totalMoney.replace(/,/g, "");
                ...
                <div className="flex items-center ">
                    <Field>
                        ...
                        <InputPost
                            control={control}
                            name="totalMoney"
                            value={`${money ? nf.format(newMoney?.replace(/\$|,|\./g, '')) : nf.format(newMoney?.replace(/\$|,|\./g, ''))
