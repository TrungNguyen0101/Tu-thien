...
const UpdateUser = () => {
    ...
    console.log(
        "🚀 ~ file: updateuser?.account?.js ~ line 14 ~ UpdateUser ~ user",
        user
    );
    ...
    const handleAddPost = (values) => {
        ...
        newValue.username = user?.account?.username
        newValue.roleId = user?.account?.roleId
        console.log(
            "🚀 ~ file: updateuser?.account?.js ~ line 57 ~ handleAddPost ~ newValue",
            newValue
        );
        handleCallAPIAddPost(
            `http://localhost:8080/api/auth/UpdateAccountByUser?idAccount=${user?.account?.id}`,
            newValue
        );
                ...
                <div className="flex items-center ">
                    <Field>
                        ...
                        <InputPost
                            ...
                            name="totalMoney"
                            value={`${!money
                                ? nf.format(user?.totalMoney.replace(/\$|,|\./g, ''))
                                : money
                                } VNĐ`}
                            className=""
                            disabled="disabled"
