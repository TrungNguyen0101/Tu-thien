...
const ItemSupport = ({ item, ...props }) => {
    ...
    const handleSubmitParticipation = async () => {
        const check = await handleGetParticipantByUserID(`http://localhost:8080/api/participant/getParticipantByUserId?userId=${userID}`);
        console.log("🚀 ~ file: ItemSupport.js:172 ~ handleSubmitParticipation ~ check", check)
        check.length > 0 && check.forEach((item) => {
            console.log("🚀 ~ file: ItemSupport.js:173 ~ check.forEach ~ item", item)
