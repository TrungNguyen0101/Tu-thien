...
const ItemSupport = ({ item, ...props }) => {
    ...
    console.log("🚀 ~ file: ItemSupport.js ~ line 11 ~ ItemSupport ~ donate", donate.length)
    ...
    useEffect(() => {
        ...
        donate.length > 0 && donate.forEach(object => {
            handleGetDonateByDonateId(`http://localhost:8080/api/donate/getDonateByDonateID?donateId=37`)
            console.log(object);
        });
    }, [item.id]);
    return (
