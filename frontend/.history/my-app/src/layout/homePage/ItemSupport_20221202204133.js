...
const ItemSupport = ({ item, ...props }) => {
    ...
    console.log("🚀 ~ file: ItemSupport.js ~ line 16 ~ ItemSupport ~ account", account)
    ...
    useEffect(() => {
        handleGetAccountByID(`http://localhost:8080/api/auth/getAccountByID?accountId=${userLead}`)
        handleGetParticipantByID(`http://localhost:8080/api/participant/getParticipantById?userId=${userID}`)
        console.log("🚀 ~ file: ItemSupport.js ~ line 179 ~ useEffect ~ userID", userID)
    }, [userID, userLead])
    // useEffect(() => {
    ...
    return (
        <div className="itemSupport-item border-2 flex flex-col items-center duration-200 mb-[20px]">
            ...
            <div className="pb-[10px] mt-auto flex flex-col items-center">
                ...
                >
                    <span className="">{`Ngày bắt đầu: ${formatDateStart}`}
                        <h3 className="inline text-[red] font-semibold">{formatDateStart}</h3>
                    </span>
                    <span className="">{`Ngày kết thúc: ${formatDateEnd}`}</span>
                    <span className="">{`Số người tham gia dự kiến: 0/5`}</span>
                    <div className="mt-[10px]">
                        <span className="inline">{`Phụ trách bởi :`}
                            <h3 className="inline text-[red] font-semibold">{account?.displayName
                                ? account?.displayName
