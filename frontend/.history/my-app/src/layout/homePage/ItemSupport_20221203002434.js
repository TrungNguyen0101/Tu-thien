...
const ItemSupport = ({ item, ...props }) => {
    ...
    console.log("🚀 ~ file: ItemSupport.js:19 ~ ItemSupport ~ countParticipantByPostID", countParticipantByPostID)
    ...
    const handleSubmitParticipation = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (participantByID?.lead === null && countParticipantByPostID < item.totalParticipant) {
                    const check = await handleAddParticipant(
                        "http://localhost:8080/api/participant/addParticipant",
                        {
                            userID: userID,
                    ...
                    Swal.fire(
                        "Thất bại",
                        "Bạn đang tham gia một dự án nào trước đó hoặc số lượng đã đủ nên không thể tham gia tiếp nữa.",
                        "error"
                    ).then(function () {
                        navigate("/");
                        window.location.href = window.location.href;
