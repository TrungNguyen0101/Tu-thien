...
const ItemSupport = ({ item, ...props }) => {
    ...
    console.log("🚀 ~ file: ItemSupport.js:29 ~ ItemSupport ~ formatDateStart", formatDateStart)
    const numberOfDaysToAdd = 30;
    const result = date.setDate(date.getDate() + numberOfDaysToAdd);
    const result1 = new Date(result)
    const formatDateEnd = new Date(result).toLocaleDateString("vi");
    console.log("🚀 ~ file: ItemSupport.js:34 ~ ItemSupport ~ formatDateEnd", formatDateEnd)
    const start = format(date, 'yyyy/MM/dd');
    console.log("🚀 ~ file: ItemSupport.js:33 ~ ItemSupport ~ start", start)
    const end = format(result1, 'yyyy/MM/dd');
    console.log("🚀 ~ file: ItemSupport.js:35 ~ ItemSupport ~ end", end)
    // const startDate = new Date(start);
    // const endDate = new Date(end);
    const difference = differenceInDays(end, start);
    console.log("🚀 ~ file: ItemSupport.js:32 ~ ItemSupport ~ difference", difference)
    console.log("123" + formatDateEnd - formatDateStart);
