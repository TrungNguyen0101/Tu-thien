...
const ItemSupport = ({ item, ...props }) => {
    ...
    const handleSubmitAdd = async () => {
        ...
        kq?.length > 0 && kq.forEach((item) => {
            console.log("🚀 ~ file: ItemSupport.js:128 ~ kq?.length>0&&kq.forEach ~ item", item)
            if (item.postId !== null || item.finish === false) {
    ...
    const handleSubmitParticipation = async () => {
        ...
        const kq = await handleGetParticipantByUserID(`http://localhost:8080/api/participant/getParticipantByUserId?userId=${userID}`);
        kq?.length > 0 && kq.forEach((item) => {
            console.log("🚀 ~ file: ItemSupport.js:180 ~ kq?.length>0&&kq.forEach ~ item", item)
            if (item.postId !== null || item.finish !== true) {
                check = true;
            }
