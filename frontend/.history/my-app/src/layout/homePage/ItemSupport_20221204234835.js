...
const ItemSupport = ({ item, ...props }) => {
    ...
    const handleSubmitAdd = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (participantByID?.lead === undefined) {
    ...
    const handleSubmitParticipation = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                console.log(participantByID?.lead === undefined)
                if (participantByID?.lead === undefined && countParticipantByPostID < item.totalParticipant) {
                    const check = await handleAddParticipant(
                        "http://localhost:8080/api/participant/addParticipant",
                        {
                            userID: userID,
