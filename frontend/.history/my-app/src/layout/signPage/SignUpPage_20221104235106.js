...
const SignUpPage = () => {
	...
	const onSubmit = async (values) => {
		if (isValid) {
			...
			const check = await handlePostAccount("http://localhost:8080/api/auth/addAccount", values)
			console.log("🚀 ~ file: SignUpPage.js ~ line 139 ~ onSubmit ~ check", check)
			if (check) {
			...
			>
				<div className="material-button alt-2 " ref={ref}>
					<span
						className="shape"
						onClick={() => {
							setWatchRadio(getValues("role_name"))
							console.log("role_name" + getValues("role_name"))
							console.log("🚀 ~ file: SignUpPage.js ~ line 18 ~ SignUpPage ~ watchRadio", watchRadio)
