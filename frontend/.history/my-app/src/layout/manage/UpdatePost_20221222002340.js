...
const UpdatePost = () => {
	...
	const handleUpdate = (values) => {
		...
		const totalMoney = values.totalMoney;
		if (isNaN(totalMoney)) {
			// Gán lỗi cho trường totalMoney
			setError('totalMoney', {
				type: 'manual',
				message: 'totalMoney must be a `number` type, but the final value was: `NaN` (cast from the value `""`).'
			});
		}
		setTimeout(() => {
			navigate("/quan-ly");
