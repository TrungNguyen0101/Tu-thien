...
const UpdatePost = () => {
    ...
    const schema = yup
        .object({
            ...
            totalParticipant: yup.number().min(limit: countParticipantByPostID, `Phải nhập hơn số lượng người đang tham gia: ${countParticipantByPostID} `).required("Vui lòng nhập số lượng người tham gia"),
        })
        .required();
    const {
        control,
