...
const UpdatePost = () => {
	...
	const number = post?.totalMoney.toString()?.replace(/\$|,|\./g, "");
	...
	const schema = yup
		.object({
			...
			totalParticipant: yup
				...
				.typeError('Vui lòng nhập vào con số'),
			date: yup
				.number()
				.min(
					0, "Vui lòng nhập số ngày lớn hơn 0"
				)
				.required("Vui lòng  số ngày của dự án").typeError('Vui lòng nhập vào con số'),
			totalMoney: yup
				.number()
				.min(
					0, "Vui lòng nhập số tiền lớn hơn 0 "
				)
				.required("Vui lòng  số tiền cần huy động")
				.typeError('Vui lòng nhập vào con số'),
	...
	console.log("🚀 ~ file: UpdatePost.js:64 ~ UpdatePost ~ isValid", isValid)
	console.log("🚀 ~ file: UpdatePost.js:64 ~ UpdatePost ~ errors", errors)
	...
	useEffect(() => {
		const getPostById = async () => {
			...
			setValue("totalMoney", money);
		};
		getPostById();
	}, [post?.desc, post?.hot, post?.image, post?.title, post?.totalParticipant, post?.date, postId, setValue, money]);
	const handleUpdate = (values) => {

		console.log(post?.image);
		let newValue = { ...values };
		newValue.totalMoney = parseInt(values.totalMoney)
		console.log("🚀 ~ file: UpdatePost.js:134 ~ handleUpdate ~ newValue", newValue)
		if (!values?.image_url) {
		...
		<div className="">
			...
			>
				<div className="grid grid-cols-2 gap-x-10 mb-10">
					...
					<Field>
						<Label>Số tiền cần huy động</Label>
						<InputPost
							control={control}
				...
				<div className="text-center pb-[30px]">
					<button
						...
						type="submit"
						className={`cursor-pointer font-medium p-[10px] border-[2px] border-transparent bg-[#00b6f1]  text-white duration-300 text-[15px] text-center rounded-[10px]  w-[150px]  " hover:bg-white hover:text-[#00b6f1] hover:border-[#00b6f1] " 
							} `}
					// onClick={() => {
