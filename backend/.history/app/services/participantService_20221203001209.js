...
const getCountParticipantByPostIdService = async (query) => {
    const { postId } = query
    console.log("🚀 ~ file: participantService.js:30 ~ getCountParticipantByPostIdService ~ postId", postId)
    const participant = await Participant.count({
        where: {
            postId: postId
        }
    });
    console.log("🚀 ~ file: participantService.js:36 ~ getCountParticipantByPostIdService ~ participant", participant)
    return participant;
};
