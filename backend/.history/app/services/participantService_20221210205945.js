...
const getAllParticipantByPostIdService = async (query) => {
    const { postId } = query
    const participant = await Participant.findAll({ where: { postId: postId, finish: false } });
    return participant;
};
...
const UpdateFinishByPostIdService = async (query, body) => {
    ...
    const participant = await Participant.update({ postId: null }, { where: { postId: postId } });
    return { post, participant, message: "Cập nhập thành công" }
}
module.exports = {
