...
const getDonateByDonateIDService = async (query) => {
    ...
    const postDonate = await db.sequelize.query("SELECT users.name AS user, products.name AS favorite FROM users JOIN products ON users.favorite_product = products.id", { type: QueryTypes.SELECT });
    return postDonate;
};
module.exports = {
    addDonateService,
