...
const UpdatePasswordService = async (query, body) => {
    try {
        const { OldPassword, OldPassword } = body;
        const { username } = query
        const account = await Account.findOne({ where: { username: username } });
        if (account) {
            const isCheck = await bcrypt.compare(passwordOld, account.password);
            console.log("🚀 ~ file: authService.js:78 ~ UpdatePasswordService ~ isCheck", isCheck)
            if (isCheck) {
                await Account.update({ password: password }, { where: { username: username } });
