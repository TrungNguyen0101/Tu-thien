...
const getDonateByDonateIDService = async (query) => {
    const { id, donateId } = query
    // const postDonate = await Donate.findAll({ where: { id: donateId } });
    const postDonate = await db.sequelize.query("SELECT donates.money AS donates,FROM donates JOIN postdonates ON :id = :donateId",
        {
            replacements: { id: id, donateId: donateId },
